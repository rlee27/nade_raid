...
class Player extends Sprite {
    ...
    if (playerRight > wallLeft && playerRight / 2 < wallRight / 2) {
      if (playerBot > wallTop && playerBot < wallBot) {
        if (Math.round(playerRight) <= wallLeft + 2) {
          console.log("left-wall-top");
          this.pos[0] = wallLeft - this.dWidth;
          this.pos[1] -= 2;
        } else {
          console.log("top-wall-left");
          this.pos[0] -= 2;
          this.pos[1] = wallTop - this.dHeight;
        }
      } else {
        if (Math.round(playerRight) <= wallLeft + 2) {
          console.log("left-wall-bot");
          this.pos[0] = wallLeft - this.dWidth;
          this.pos[1] += 2;
        } else {
          console.log("bot-wall-left");
          this.pos[0] -= 2;
          this.pos[1] = wallBot;
        }
      }
      ...
      if (playerBot > wallTop && playerBot < wallBot) {
        if (Math.round(playerLeft) >= wallRight - 2) {
          console.log("right-wall-top");
          this.pos[0] = wallRight;
          this.pos[1] -= 2;
        } else {
          console.log("top-wall-right");
          this.pos[0] += 2;
          this.pos[1] = wallTop - this.dHeight;
        }
      } else {
        if (Math.round(playerLeft) >= wallRight - 2) {
          console.log("right-wall-bot");
          this.pos[0] = wallRight;
          this.pos[1] += 2;
        } else {
          console.log("bot-wall-right");
          this.pos[0] += 2;
          this.pos[1] = wallBot;
        }
      }
    }
    //  } else if (playerBot > wallTop &&
    //             playerRight > wallLeft &&
    //             playerRight < wallRight &&
    //             playerTop < wallTop) {
  ...
  nearestCell() {
    const cellCoords = [];
    const minX = this.dx - 50;
    const minY = this.dy - 50;
    const maxX = this.dx + 50;
    const maxY = this.dy + 50;
    let xCoord, yCoord;
